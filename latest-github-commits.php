...
class Latest_Github_Commits extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (false === ($results = get_transient('latest_github_commits_json'))) {
			...
			$url = 'http://github.com/api/v2/json/commits/list/'.urlencode($instance['github_user']).'/'.urlencode($instance['github_project']).'/master';
		...
		if(isset($results['error']))
		{
			echo "<div class='pod'>";
			echo "<h6>".$title."</h6>";
			echo "<p>Error fetching from Github: <strong>".$results['error']."</strong>. This error is cached so not to hammer Github (but when you update the widget settings cache is cleared)</p>";
			echo "</div>";
		}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Latest Github Commits', 'latest_commits'), 'Github User' => __('Github User', 'github_user'), 'Github Project' => __('Github Project', 'github_project'), 'Max commits' => 5);
		...
		<p>
			...
			<input id="<?php echo $this->get_field_id( 'github_user' ); ?>" name="<?php echo $this->get_field_name( 'github_user' ); ?>" value="<?php echo $instance['github_user']; ?>" style="width:95%;" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'github_project' ); ?>"><?php _e('Github Project:', 'hybrid'); ?></label>
			<input id="<?php echo $this->get_field_id( 'github_project' ); ?>" name="<?php echo $this->get_field_name( 'github_project' ); ?>" value="<?php echo $instance['github_project']; ?>" style="width:95%;" />
		</p>
